...
local function _derive_session_key(s)
	if not crypto.available or not crypto.ecdh_available then
		return nil
	end
...
function M.new(config)
	...
	function self:_set_state(s)
		...
		if s ~= prev then
			schedule(function()
				self:_emit("state_change", s, prev)
			end)
		end
	end
	...
	function self:gather(callback)
		...
		ice.gather({ port = config.port, stun = config.stun }, function(err, cands, handle)
			if err then
			...
			-- relay_is_consumer: this peer connects to the remote's relay broker; skip generating
			-- own relay_token/candidate (the remote's token is read from their description instead).
			local desc_cands = cands
			if config.relay and not config.relay_is_consumer then
				desc_cands = {}
				for _, c in ipairs(cands) do
					desc_cands[#desc_cands + 1] = c
				end
				...
				if relay_cand then
					desc_cands[#desc_cands + 1] = relay_cand
				end
			end
	...
	function self:_try_relay(direct_err)
		...
		if not relay_url or not relay_token then
			...
			schedule(function()
				self:_emit("close", direct_err)
			end)
			return
		end

		relay_m.connect(relay_url, relay_token, { timeout = config.relay_timeout }, function(err, relay_conn)
			if self.state == "closed" then
				return
			end -- timed out meanwhile
			if err then
				self:_set_state("closed")
				if self._handle and not self._handle:is_closing() then
					self._handle:recv_stop()
	...
	function self:_start_checks()
		...
		ice.check_pairs(pairs, self._handle, config.probe or {}, function(err, winning_pair, handle)
	...
	function self:close()
		...
		self._handle = nil
		schedule(function()
			self:_emit("close", "closed by local peer")
		end)
	end
