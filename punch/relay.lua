...
local schedule = (vim and vim.schedule) or function(fn)
	fn()
end
...
local function new_relay_conn(ws_conn)
	...
	ws_conn:on("close", function(reason)
		self._closed = true
		schedule(function()
			self:_emit("close", reason or "relay disconnected")
		end)
	end)
...
function M.make_candidate(broker_url, relay_token)
	local host = broker_url:match("^wss?://([^:/]+)")
	-- Extract port before any path component ("ws://host:PORT/path" or without port).
	local port = tonumber(broker_url:match("^wss?://[^:/]+:(%d+)")) or 80
	if not host then
